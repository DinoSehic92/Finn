...
{
    private bool _suppressAutoGeneratedThemes;
    ...
    public xColorDia()
    {
        ...
        var darkBackgroundPicker = this.FindControl<ColorPicker>("BackgroundColorPickerDark");
        if (darkBackgroundPicker != null)
        {
            darkBackgroundPicker.PropertyChanged += (_, e) =>
            {
                if (e.Property != ColorPicker.ColorProperty || _suppressAutoGeneratedThemes) return;
                if (this.DataContext is MainViewModel vm)
                {
                    var color = darkBackgroundPicker.Color;
                    vm.UI.AutoGenerateDarkThemeFromBackground(color);
                }
            };
        }

        var lightBackgroundPicker = this.FindControl<ColorPicker>("BackgroundColorPickerLight");
        if (lightBackgroundPicker != null)
        {
            lightBackgroundPicker.PropertyChanged += (_, e) =>
            {
                if (e.Property != ColorPicker.ColorProperty || _suppressAutoGeneratedThemes) return;
                if (this.DataContext is MainViewModel vm)
                {
                    var color = lightBackgroundPicker.Color;
                    vm.UI.AutoGenerateLightThemeFromBackground(color);
                }
            };
        }
    ...
    public void OnDarkPresetSelected(object? sender, SelectionChangedEventArgs e)
    {
        ...
        _suppressAutoGeneratedThemes = true;
        try
        {
        ...
        {
            _suppressAutoGeneratedThemes = false;
        }
    }
